...
real (DP), DIMENSION(:,:), ALLOCATABLE :: slipz1_inter,slipz1_cos,v_cos,slip_cos,&
      slipz1_tau,slipz1_v,slipz1_seff,seff_inter,xLf_inter
...
!!! modify output number
   ALLOCATE (slipz1_inter(Nt_all,nas),slipz1_cos(Nt_all,ncos), &
            ...
            slipz1_seff(Nt_all,ncos),xLf_inter(Nt_all,nas))
...
do j=1,Nt_all !! source
 ...
 !stiff(i,j)=stiff(i,j)  !change the sigh of stiff from negative to positive 
 !if (stiff(i,j).gt.1.5)then !remove the abnomrl large cell from stiff matrix
 !   stiff(i,j)=0.001
 !end if
end do
end do
...
!--------lms fault model -93-180km
         yt(2*j-1)=(1.536d-7)*(x(j)**3)-(2.777d-5)*(x(j)**2)-0.00643d0*x(j)+1.967d0
	 yt(2*j)=xLf(j)/(1.1d0*yt(2*j-1))	
         slip(j)=0.d0
...
REAL (DP),  parameter :: A2=.2d0,A3=.3d0,A4=.6d0,A5=1.d0,A6=.875d0, &
                  B21=.2d0,B31=3.d0/40.,B32=9.d0/40.,B41=.3d0,&
                 B42=-.9d0,B43=1.2d0,B51=-11.d0/54.,B52=2.5d0, &
                 B53=-70.d0/27.,B54=35.d0/27., B61=1631.d0/55296., &
                 B62=175.d0/512.,B63=575.d0/13824.,B64=44275.d0/110592., &
                 B65=253.d0/4096.,C1=37.d0/378., C3=250.d0/621.,  &
                 C4=125.d0/594.,C6=512.d0/1771.,DC1=C1-2825.d0/27648., &
                 DC3=C3-18575.d0/48384.,DC4=C4-13525.d0/55296.,  &
                 DC5=-277.d0/14336.,DC6=C6-.25d0
...
!----------lms fault model -93-180km
           dd=(1.536d-7)*(x(i)**3)-(2.777d-5)*(x(i)**2)-0.00643d0*x(i)+1.967d0-0.001d0
          zz(i)=yt(2*i-1)-dd
...
do i=1,Nt_all
   seff_all(i)=28.d0+180.d0*z_all(i)
...
real (DP) :: slipz1_inter(Nt_all,nas),slipz1_cos(Nt_all,ncos),&
        ...
        seff_inter(Nt_all,nas),xLf_inter(Nt_all,nas)
